...
func rootCommand(ctx *commandContext) *cobra.Command {
	root := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Name() == "login" || cmd.Name() == "bootstrap-login" {
				return nil
			}
			...
			client, err := libclient.LoadOrBootstrapConfig(
				...
				strings.TrimSpace(viper.GetString("octopus_api_key")),
			)
			if err != nil {
				return fmt.Errorf("unable to load session: %w\nRun: OCTOPUS_API_KEY=... octopus-cli login", err)
			}
	...
	root.AddCommand(bootstrapLoginCommand())
